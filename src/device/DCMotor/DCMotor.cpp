...
void setSpeed(DCMotor* motor, float speed) {
    if (speed > 0 && speed <= 1) {
        analogWrite(motor->hardwareDescriptor.pins[DCMotor_PWM_Pin1], speed * 255);
        analogWrite(motor->hardwareDescriptor.pins[DCMotor_PWM_Pin2], 0);
    } else if (speed < 0 && speed >= -1) {
        analogWrite(motor->hardwareDescriptor.pins[DCMotor_PWM_Pin1], 0);
        analogWrite(motor->hardwareDescriptor.pins[DCMotor_PWM_Pin2], speed * -255);
    } else {
        analogWrite(motor->hardwareDescriptor.pins[DCMotor_PWM_Pin1], 0);
        analogWrite(motor->hardwareDescriptor.pins[DCMotor_PWM_Pin2], 0);
    }
